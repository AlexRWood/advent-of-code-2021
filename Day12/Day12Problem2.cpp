...
int navigate(std::unordered_map<std::string, Cave> &listOfCaves, std::unordered_map<std::string, bool> &visitedCaves, Cave &currentLocation, bool smallCaveVisited);

int main() {
    int testResult {solution("test1.txt")};
    assert(testResult == 36);
    testResult = solution("test2.txt");
    assert(testResult == 103);
    testResult = solution("test3.txt");
    assert(testResult == 3509);
...
int solution(std::string s) {
    ...
    score = navigate(listOfCaves, visitedCaves, listOfCaves.at("start"), false);
    return score;
}
...
int navigate(std::unordered_map<std::string, Cave> &listOfCaves, std::unordered_map<std::string, bool> &visitedCaves, Cave &currentLocation, bool smallCaveVisited) {
    ...
    for(auto &cave : currentLocation.adjacentCaves) {
        if(cave.isSmall && visitedCaves.find(cave.name) == visitedCaves.end()) {
            ...
            score += navigate(listOfCaves, newVisitedCaves, listOfCaves.at(cave.name), smallCaveVisited);
        }
        else if(cave.isSmall && visitedCaves.find(cave.name) != visitedCaves.end() && !smallCaveVisited && cave.name != "start") {
            score += navigate(listOfCaves, visitedCaves, listOfCaves.at(cave.name), true);
        }
        else if(!cave.isSmall) {
            score += navigate(listOfCaves, visitedCaves, listOfCaves.at(cave.name), smallCaveVisited);
        }
    }
    return score;
}